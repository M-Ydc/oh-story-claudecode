...
| 位置 | 类型 | 原文 | 问题 |
|------|------|------|------|
| 第X段 | 模板叠加 | "他缓缓站起身，眼中闪过一丝..." | 2+信号词机械叠加 |
| 第Y段 | 句式 | "...，带着..." | AI惯用句式 |
| 第Z段 | 节奏 | 连续3句排比 | 过于工整 |
| ... | 心理描写 | "他感到..." | 告诉而非展示 |
...
|----------|---------|------|----------|
| 轻度 | 模板叠加句 ≤3 处/千字，无连续 3+ 句式套路 | 少量信号词单用，偶有书面腔 | 只过 Gate A + B |
| 中度 | 模板叠加句 4-8 处/千字，或有连续 3+ 句式套路 | 多处模板叠加 + 句式套路 + 心理描写抽象 | 过 Gate A + B + C + D |
| 重度 | 模板叠加句 >8 处/千字，或 6 Gate 中 4+ 个有问题 | 全文AI味明显，节奏/对话/结尾都有问题 | 完整 6 Gate + 重点段落重写 |

> 量化标准为参考值。模板叠加句 = 同一句中出现 2+ 模板信号词（banned-words.md 中的信号词）机械叠加。如果计数与主观判断冲突，以定性描述为最终裁定依据。

**AI味打分客观指标**：

| 指标 | 计算方式 | 轻度阈值 | 中度阈值 | 重度阈值 |
|------|----------|---------|---------|---------|
| 模板叠加密度 | 2+信号词叠加句数 / 千字 | ≤3 | 4-8 | >8 |
...
加载 [references/banned-words.md](references/banned-words.md)，扫描每句话中的模板信号词叠加情况。

**核心规则**：单用不禁，叠加必杀。模板信号词（缓缓/一丝/轻轻/仿佛/眼中闪过/心中一动等）单独使用且在语境中有功能 → 保留。同一句中 2+ 信号词机械叠加 → AI模板 → 改写。

**检测方法**：扫描每句话，统计句中信号词数量：
- 0-1 个 = 安全（保留）
- 2 个 = 检查是否模板句式，若是→改写
- 3+ 个 = 必定AI模板，拆成短句、删掉无功能的词

**白名单机制**：

项目根目录下的 `.deslop-whitelist` 文件定义了本项目的豁免词汇（一行一个）。
如果该文件存在，其中的词汇在 Gate A 检查时不被计入信号词。

白名单匹配规则：精确匹配（非子串）。

白名单适用场景：
- 命中术语（如玄幻小说中的特定术语恰好匹配信号词）
- 角色口头禅（某角色的标志性用语）
- 世界观专有名词
- 原文刻意使用的修辞手法

如果 `.deslop-whitelist` 不存在，不强制创建文件；在报告中说明可创建该文件。只有项目目录可写且本次需要复用白名单时，才创建空白名单文件。空白名单文件等同于无白名单，所有信号词正常计入。


**保护规则优先级**：保留创作意图与剧情功能 > 去AI Gate。Gate A-F 只能改变表达方式，不能删除伏笔、钩子、角色特征、关键信息或必要转折；遇到冲突时改为降AI重写或标注 `[需复核]`。

改写规则：
- 模板叠加句 → 拆成2-3个短句，每句最多1个信号词
- 无功能的信号词 → 删除或替换为具体动作
- 不能简单换成另一个形容词，要用"展示"替代"告诉"

示例：
- ❌ "他缓缓站起身，眼中闪过一丝不易察觉的悲伤"（3个信号词叠加） → ✅ "他站起来。垂下眼。"
- ❌ "她微微一愣，心中涌起一股暖流，不由得笑了"（3个信号词叠加） → ✅ "她愣了下。笑了。"
...
| "仿佛/犹如/宛若" | 文言腔过重，叠加时需处理 | 口语化表达或白描；单独用于角色视角内的不确定感知可保留 |
...
| 文件 | 何时加载 |
|------|----------|
| [references/banned-words.md](references/banned-words.md) | 模板叠加检测（Gate A）时 |
