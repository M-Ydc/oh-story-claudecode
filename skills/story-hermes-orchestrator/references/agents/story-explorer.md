...
> 来源：oh-story-claudecode/skills/story-setup/references/templates/agents/story-explorer.md
> 此文件为 agent 系统提示词正文，已去除 Claude Code 专有 frontmatter
...
6. **章节基调候选集**：
   ...
   - 对每个文件 `Grep -hE '基调：(紧张|轻松|悲伤|热血|温馨|压抑)'`（**全角冒号**，不锚定行首）拿到该章所有情节点基调
   - 章基调聚合：众数；并列时按 grep 输出顺序取最早
   - 候选集 = 章基调 == 本章情绪/基调的章节列表
7. **相近基调兜底**（完全没有同基调章节时）：
   - 先从本章细纲/查询参数里判断更接近“紧张、热血、轻松、温馨、悲伤、压抑”哪一类；不要写固定矩阵。
