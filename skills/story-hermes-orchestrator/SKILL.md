...
- **context 模板**：
  ...
  呼吸法规则（来自 meisi 第十四节）：
  - 锚点：每个场景有一个物理锚点（物件/数字/声音/味道），读者手里有东西可以摸
  - 层叠：信息逐层铺开（地点→氛围→位置→动作→境遇→愿望），不是一次性倒出来
  - 反刍：同一信息/动作出现两次，第二次不为推进——为加深。至少1次/章
  - 停顿：重大转折前必须有呼吸停顿（安静有重量的段落），不是"然后"
  - 翻译：meisi 技法可以翻译不能照搬。问"这个技法在原作中的情绪功是什么？等效到我这里是什么？"翻不出来就删
