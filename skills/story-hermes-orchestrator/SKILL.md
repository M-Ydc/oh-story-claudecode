...
- **context 模板**：
  ...
  - **高潮时刻除外**：本章高潮（觉醒/战斗/关键释放场景）可以有密集感官爆发，这里是读者等的关键释放点。不要在高潮时刻砍感官
  ...
  呼吸模式（来自 meisi 第十四节——五档变速。本章使用 {breathing_mode} 模式）：

  ⚠️ 模式规则优先级高于下方通用规则。当模式规则与降维/感官/节奏规则冲突时，
  以模式规则为准。

  各模式约束如下：

  建立模式（第1章/新卷首章）：
  - 6层逐层铺开：地点→氛围→位置→动作→境遇→愿望，每层一句话
  - 物理锚点（物件/数字/声音/味道）贯穿全场景
  - 85行以上角色相处时间再进事件
  - 字数可以适度超标
  ...
  - 1-3句场景定调即入情节，不重建角色
  - 物理锚点可有可无，反刍不需要
  - 不要因为"写得好"而突破字数上限——流动模式是效率模式

  语调模式（日常/过渡章）：
  - 开篇叙述者声音先于场景（哲学/喜剧/反思）
  - 降维评论和黑色幽默密度可提升
  - 反刍改为"回味"——叙述者换角度吐槽同一件事
