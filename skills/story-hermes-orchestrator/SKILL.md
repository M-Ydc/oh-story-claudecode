...
- **context 模板**：
  ...
  === 写作结构（本章使用 {breathing_mode} 模式。按 Beat 序列执行，预算用尽即停）===

  建立模式（第1章/新卷首章）：
  Beat 1 锚点 ~60字：地点→氛围→主角位置。一个具体动作。参照"天琅国，柳州，陈家府邸。今天依旧很热闹……王易却一个人躲在厨房里偷懒，嘴里叼着根菜梗，没滋没味的嚼着。"不要"却"碰瓷——没有等效情绪就不要用对照词。
  Beat 2 境遇 ~80字：信息逐层铺。催款→算账(1遍)→身体反应→底下压着什么。愿望要小("还差两千多"不是"我想解决问题")。同一信息团最多铺2次，第3次必须是新信息或新角度。算账只算1遍——反刍不是把同一道减法做3遍。
  Beat 3 日常 ~200字：全场景绕物理锚点转。室友对话+小物件细节(红牛罐/棒棒糖/糖渍)+背景噪音(拖鞋/警笛/空调)。对话留白1处。此处放第1次降维评论(先拉高再砸下，≤15字)。
  Beat 4 停顿 ~40字：安静有重量。油花凝了。不推进任何信息。
  Beat 5 切入 ~20字：短脆独立段。玻璃碎了。不要预告不要铺垫。
  Beat 6+ 走廊→追逃→觉醒→警笛→抉择。追逃段剃光感官只留动作。觉醒高潮感官密集爆发。章尾去留抉择悬停——不选跑也不选留。结尾用动作/细节收束。

  流动模式（常规章）：
  开篇 ~30字：1-3句场景定调。不重建角色。直入情节。字数不超过细纲目标110%。

  语调模式（日常/过渡章）：
  开篇 ~50字：叙述者声音先于场景。有趣。降维幽默密度提升。

  动作模式（高潮/战斗章）：
  零呼吸开篇。追逃剃感官。高潮不降维。

  收束模式（卷末/弧线收尾/死亡/离开）：
  最短句。最大留白。不解释情绪。禁降维。章尾句号。

  === 写作后自检（写完逐条过，不嵌入写作流程）===
  - 去AI味：Gate A禁用词/Gate B排比/Gate C情绪→身体/Gate D同构句/Gate E对话差异/Gate F禁升华章尾
  - 时间线：本章身份状态与细纲一致，未提前泄露后续身份
  - 人设：性格通过场景选择展示，无旁白插叙
  - meisi：技法翻译情绪功，翻不出就删。「每…都…」≤2次
  - 动线：下楼方向、妖兽造型、细纲外动作三重验证
