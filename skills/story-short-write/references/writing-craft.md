...
写完一段后，用"三指遮盖法"检查：分别遮住发生信息、感知信息、反应信息，看每个维度是否都有内容。如果某个维度缺失，织入对应细节——而不是在后面另起一段补写。

### 镜头断段（防止一段到底）

三维度织入不是把一个子事件塞进一个长段。织入解决"同一瞬间被拆成发生/感知/反应三段"的问题；断段解决"手机阅读时一段过密"的问题。

断段按**注意力瞬间**切换，不按维度变化，也不按句号机械拆段。

**"注意力瞬间"是什么**：读者阅读时，大脑一次聚焦于一个感知单元。看见一个动作、听见一句话、感受到一个身体反应、闪过一个念头——每个都是一次注意力瞬间。段落的边界应该对齐这些注意力切换点。同一个注意力瞬间内的连续内容合为一段；注意力一切换就换段。这与本 skill 的"镜头式写作法"（style-craft.md）一致：一个镜头 = 一个注意力瞬间 = 一个段落。

详细的注意力合段规则和示例见 [format-and-structure.md](format-and-structure.md) "核心规则：注意力瞬间合段"。
...
硬规则（与 story-format.md "注意力瞬间合段"原则一致）：

- 一段承载一个注意力瞬间——同一镜头内的连续动作/感知/反应合为一段，段内用逗号串联。
- 注意力切换时换段：新动作开始、新物件/线索出现、视线转移、直接引语出现、心理定格/判断落下。
- 单段超过 60 字时，优先按注意力切换点拆开（手机阅读：每行约15字，4行已是较大段落）。
- 单句超过 35-45 字时，除非刻意放慢节奏，否则在动作转折或视线转移处拆成两句。
- 不要连续多段同长度：短句、稍长句、对话、动作段交替。
- 连续 4 个以上 ≤12 字短段时，检查是否同一镜头被机械拆碎——若是则合并相邻两句。

输出前做一次密度重排：

1. 扫描每个自然段；一段 >60 字，检查是否有隐藏的注意力切换点（新动作/新物件/新信息），有则在该处换段。
2. 如果单句 >45 字，在逗号、动作转折、视线转移处拆成两句，或删掉多余修饰。
3. 连续 4 个以上 ≤12 字短段时，只合并同一镜头内的相邻两句，避免碎成诗。不同镜头间即使都短也不合并。
4. 朗读检查：朗读全文，在自然"换气"处确认断段合理。

错误修法：把织入段拆成"动作段/感知段/反应段"按维度分段。或机械按"3个句号必须换行"拆碎同一注意力瞬间。

正确修法：保留同一瞬间的织入关系，但在注意力切换（新动作、新物件、新信息、新对话）处断开。
