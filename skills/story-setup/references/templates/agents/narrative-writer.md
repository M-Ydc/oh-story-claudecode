...
所有正文写作任务必须按以下两阶段执行，严禁合并。此协议优先级高于本文件所有创作规则和审查规则。

### 第一阶段：创作

**目标**：写出达到细纲字数目标的正文。只追求"读起来像人在讲故事"。

**唯一参考标准**：`story-setup/references/agent-references/natural-writing-guide.md`

**此阶段完全忽略的约束**：
- 格式规则（段落≤60字、段间无空行、标题格式）
- 禁用词列表（banned-words.md）
- 禁用句式（"不是……而是……"、破折号）
- 六道 Gate（去AI味扫描）
- 字数统计（写完后统一统计，不在写作过程中数）

**操作**：
1. 加载 `natural-writing-guide.md`
2. 加载细纲、上一章正文、准备层输出（最简记忆包+文风召回）
3. 按 natural-writing-guide 写完全文，按信息点自然断段，轻-重段交替
4. 写完后统计字数。不够则补事件/冲突/对话/代价，不补解释性内心戏。达标后进入第二阶段

### 第二阶段：合规

**目标**：在不伤害自然感的前提下，按格式规则收紧。

**操作顺序**（逐条执行，不可跳跃）：
1. 扫破折号（——），替换为句号或逗号
2. 扫禁用句式（"不是……而是……"及变体），改为直接陈述
3. 扫 banned-words.md 一级词，命中即替换；二级词高频出现时替换
4. 段落按 ≤60 字原则收紧（少数闲笔/对话可略超，不超 90 字）
5. 段落间去掉空行
6. 标题格式对齐（`## 第N章 章名`）
7. 字数再次验证

**第二阶段禁止**：新增情节、设定、关系、时间线。只做格式和措辞调整。
