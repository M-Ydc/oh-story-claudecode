...
| `story-long-write/references/style-reference-meisi.md` | **每次正文写作前必须加载**（叙述者人格注入：降维评论、零句断句、口语化叙述、黑色幽默、对话留白、信息压制）。如果找不到此路径，尝试 Glob 搜索 `*/style-reference-meisi.md`；如果项目内不存在，跳过并继续正常写作。 |
...
> 详细技法见 `story-long-write/references/style-reference-meisi.md`（如存在）

**每次写正文前，检查项目中是否存在风格参考文件 `style-reference-meisi.md`。如存在，必须在动笔前加载，并将其作为写作约束。**

风格约束摘要（详细内容以参考文件为准）：
1. **降维评论**：每 300 字至少出现一次叙述者口语化评价或吐槽（非角色对话，是第三人称叙述主体的声音）。评论方向与叙事方向相反（严肃→搞笑，悲壮→轻描淡写）。**与镜头式写作法的关系**：降维评论是"限知视角下的叙述者人格化"——叙述者替主角说出心里话，但不透露主角不知道的信息。不同于被禁的全知叙述（如"此时他还不知道..."），降维评论始终锚定在主角的当前知识和情绪范围内。
2. **零句断句**：紧接长句后用 2-6 字短句收束。对话允许单字/双字回应。禁止为了语法完整而补主语。降维评论和零句断句均独立成段，不混在三维度织入的叙事段中。
3. **对话留白**：每章至少一次角色不直接回答对方问题——用动作/沉默/看似无关的话回应。
4. **黑色幽默**：每章至少一次在紧张/悲伤/阴暗场景中插入一句轻飘飘的吐槽，制造情绪落差。
5. **信息压制**：角色发现信息时不立即解释。只写"发现"的动作和反应，细节留待后续章节。
6. **叙述者口语化**：禁止连续 500 字以上纯白描无叙述者声音介入。允许使用"靠/诶/那啥/真是/行吧/忍不住/不由得"等口语词——这些词在风格参考模式下不受 banned-words.md 禁用词表约束。
7. **章尾约束**：用具体动作/发现/问题收束。禁止"他终于明白了……""这一夜注定无人入眠""他不知道的是……"。

**风格参考模式下的禁词表优先级**：当 style-reference-meisi.md 被加载时，banned-words.md 中与口语表达冲突的条目（不禁、不由得、心中一动、忍不住、心头一震等心理类和情态类词汇）暂停生效。原因：这些词在人类作者的网文中高频出现（如口语化风格原文中"王易忍不住笑意""忍不住浮想联翩"），笼统禁用会误杀自然表达。仅保留模板化句式禁令（"眼中闪过一丝XX""嘴角勾起一抹XX"等机械套话）和二级语境敏感词的高频警戒。
