...
description: |
  ...
  情绪弧线执行、开篇/收尾、去AI味（模板叠加检测、句式去套路、节奏打碎）。
...
| `story-deslop/references/banned-words.md` | 模板叠加检测（Gate A）时 |
...
2. **展开子事件**：每个子事件将发生、感知、反应三维度织入同一段连续正文（合计 ≥100-150 字）
   ...
   - 子事件之间用身体动作连接（~20 字）
   - **镜头断段**：三维度织入不等于一段到底。按注意力瞬间断段，不按维度断段，也不按句号机械拆段。一个注意力瞬间 = 一个镜头 = 一个段落。注意力切换（新动作/新物件/视线转移/对话/心理定格）时换段
   - **手机阅读密度**：同一注意力瞬间内连续内容合为一段（≤60字）；注意力切换时断开。禁止把同一镜头内感知-反应-动作串联的完整流程拆成单独段落，也禁止把切换了注意力的内容塞进同一段
   - **输出前密度重排**：扫描每段；一段 >60 字检查是否有隐藏的注意力切换点；连续碎段像提纲时检查是否同一镜头被机械按句号拆碎。拆合的判断单位是「注意力瞬间」而非「微动作」或「句号」
3. **收尾**：钩子或情绪定格（1-2 句）
...
> 模板信号词表及叠加检测规则见 `story-deslop/references/banned-words.md`

- **Gate A 模板叠加检测**：单用不禁，叠加必杀。banned-words.md 中的模板信号词（缓缓/一丝/轻轻/仿佛/眼中闪过/心中一动等），单独使用且在语境中有功能 → 保留。同一句中 2+ 信号词机械叠加（如"他缓缓站起身，眼中闪过一丝坚定"）→ AI模板 → 拆成短句、删掉无功能的词。检测方法：扫描每句话，0-1 个信号词 = 安全；2 个 = 检查是否模板句式；3+ 个 = 必定AI模板，必须改写。
- **Gate B 句式去套路**：连续排比/刻意对称/空洞抒情打散（anti-ai-writing.md 7种AI模式检测）
- **Gate C 心理描写外化**：情绪词 -> 身体状态（anti-ai-writing.md Show Don't Tell 原则）
- **Gate D 节奏控制**：同构句打散、连续碎段合并（核心规则：机械重复的同句式结构必须打断；连续碎段像提纲时合并同一镜头内的相邻句。节奏控制靠长短句交替，不统一拆短。）
...
- 格式合规：按注意力瞬间断段，控制单段密度（≤60字）；对话独立成行；对话标签避免高频公式化，普通”说”可保留
- 节奏均匀度：是否有连续多节无情绪变化？
- 身体部位重复：同一词全文 <= 5 次
- 模板信号词叠加检测：扫描同一句中 2+ 模板信号词（缓缓/一丝/眼中闪过/心中一动等）机械叠加的模板句式
...
- **禁止模板信号词叠加**：同一句中不出现 2+ 模板信号词（缓缓/一丝/眼中闪过/心中一动等）机械叠加
- **避免信息过载**：三维度织入后不要一段到底；同一段内挤了多个不相关注意力时按注意力切换拆分。不要按微动作逐条拆散
- **禁止空转**：每个句子必须推动情节/情绪/代入感至少一项，否则删除。以下不属于空转：感官细节（气味/温度/触感）、环境氛围（光线/声音/天气）、角色即时身体反应、极短的内心定格（≤1句）、场景过渡句
