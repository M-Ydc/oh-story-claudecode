...
单个模板信号词在语境中正常使用**不是问题**。真正的AI指纹是**多个信号词在同一句中机械叠加**。

> 完整信号词表和叠加检测规则见 [banned-words.md](banned-words.md)
...
AI模板（改写）："他缓缓站起身，眼中闪过一丝坚定，嘴角勾起一抹微笑。"
→ 3个信号词叠加 + 套话结构 = AI指纹
```
...
段落按注意力瞬间断开，不是越碎越好。检查重点是朗读和手机阅读是否卡顿：

- 一段承载一个注意力瞬间（同一镜头内的连续动作/感知/反应合段）。
- 逗号串太长、多个切换了注意力的动作挤在一段里，按注意力切换点拆开。
- 连续短段碎成提纲时，检查是否同一镜头被机械按句号拆碎——若是则合并相邻句。
...
### 模板叠加警告示例（2+信号词在同一句 = 必须改写）
- "他缓缓站起身，眼中闪过一丝X" -> 拆成 "他站起来。看了她一眼。"
- "她微微一愣，心中涌起一股Y" -> 拆成 "她愣了下。没说话。"
- "仿佛Z一般，带着一丝W" -> 删掉比喻，写具体画面
...
| 单用安全（保留） | 叠加成模板（改写） |
|------|--------|
| "他缓缓转头" | "他缓缓站起身，眼中闪过一丝坚定" |
| "嘴角有一丝血迹" | "嘴角勾起一抹冷笑，心中涌起一股杀意" |
| "仿佛是错觉"（角色视角内） | "仿佛潮水般涌来"（万能比喻） |
| "她轻轻放下杯子" | "她轻轻叹了口气，微微摇头，淡淡说道" |

核心判断标准：同一句中是否出现 2+ 个模板信号词。单用不禁止，叠加必须改写。 |
...
- [ ] **段落控制**：段落按注意力瞬间断开，朗读不卡、无碎成诗或一段到底
- [ ] **模板叠加检测**：无同一句中3+信号词叠加；2信号词句中每个词有功能
