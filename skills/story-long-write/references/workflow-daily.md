...
> **日更模式准备步骤**：每章写作前执行 3 步（状态筛选 + 意图确认 + 呼吸模式），嵌入 Step 2 逐章执行循环内。日更模式省略对标结构化子目录检索（SKILL.md 步骤 3.2 的第二部分），因为题材框架已在 `设定/题材定位.md` 中固化，对标模块已在 Phase 2-3 中内化到卷纲和细纲。情绪-技法匹配（步骤 3.2 的第一部分）通过 Step 2.2 的意图确认覆盖。完整准备层逻辑见 `SKILL.md` 的 Phase 4 正文写作辅助部分。
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.4 呼吸模式**：判断本章呼吸模式（与 SKILL.md §3.4 相同的决策树）——
     - 第1章/新卷首章 → 建立模式
     - 死亡/离开 → 收束模式
     - 高潮/决战 → 动作模式
     - 过渡章 → 语调模式
     - 其余 → 流动模式
     将模式名注入 narrative-writer context，替换 `{breathing_mode}` 变量
