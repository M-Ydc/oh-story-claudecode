...
2. **逐章执行**（以下每步在每章循环内执行）：
   - **加载写作总纲**：加载 `references/natural-writing-guide.md` 作为第一优先级写作指南。当该指南与 `references/banned-words.md` 冲突时，写作阶段以 natural-writing-guide.md 为准；禁用词扫描阶段如命中 banned-words，由人工判断是否替换，不机械全删。
   - 读细纲 → 按需加载角色设定
...
3. 读取最新一章正文（情节衔接）
4. 加载 `references/natural-writing-guide.md`
5. 按 SKILL.md Phase 3 的叙事细纲模板补建本章细纲
6. 补建完成后继续 Step 2 写作
