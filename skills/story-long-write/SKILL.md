...
6.5 **风格参考加载**：检查项目中是否存在 `style-reference-meisi.md`。查找顺序：① `{项目}/对标/style-reference-meisi.md`；② Glob 搜索 `*/style-reference-meisi.md`（覆盖 skill 目录和项目子目录）。如找到，读取全文并提取约束摘要（降维评论密度、对话留白率、黑色幽默率、信息压制规则、禁升华章尾）。如未找到，跳过，按正常去AI味流程写作。
