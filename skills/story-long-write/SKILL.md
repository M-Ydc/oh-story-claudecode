...
**细纲生成时同步加载 `references/natural-writing-guide.md`**，用叙事语气写细纲。细纲的职责是"讲这一章的故事"，不是"分析这一章的结构"。事件、钩子、爽点、悬念全部融入叙事中，不单独列出字段。
...
{用叙事语气写本章内容，200-400字。像在给人讲这一章发生了什么。}
{按事件顺序写：谁做了什么，遇到什么，结果怎样。场面转换处自然分段。}
{事件推进中，读者看到了什么、感受到了什么变化——隐含在叙事里，不单独标注。}
{叙事中需要能看出：章首钩子在哪、爽点在哪、章尾钩子在哪。不用标注，但得有。}
{章尾留给读者的悬念或画面，直接在叙事最后一句点出。}

目标情绪：{交付什么情绪}
字数目标：{X} 字
```

**叙事细纲自检**：写完大声念一遍。听起来像讲故事还是像写项目汇报？如果是后者，重写。

叙事细纲仍然承担原分析式细纲的全部功能——事件顺序、钩子位置、爽点节奏、悬念设置、情绪走向——只换表达方式：

| 原分析字段 | 在叙事细纲中的位置 |
|-----------|------------------|
| 情节点序列 | 事件按发生顺序叙述 |
| 场景冲突 | 叙述中自然呈现"谁想干什么/什么挡着" |
| 信息释放 | 事件展开的节奏本身就是信息释放 |
| 情绪转折 | 叙述的紧张度变化 |
| 章首钩子 | 叙事第一句 |
| 章尾钩子 | 叙事最后一句 |
...
**细纲质量要求**：叙事摘要本身能当微型故事读。钩子、爽点、悬念都融入叙事中，不单独列字段。
...
6. **写作（两遍法·第一遍）**：以 `references/natural-writing-guide.md` 为**唯一**写作标准。核心原则：按信息单位自然分段，叙述与角色内心自然融合，对话与叙述自然衔接，用闲笔制造呼吸感，让文字密度自然变化。第 1 章如果以内心戏、设定认知或独处开场，必须先把内心变化外化为可见事件，再按字数目标展开；不得用大段心理独白凑字。若第 1 章低于目标，优先补”外部事件/对话/选择代价”，不要补解释性内心戏。

   **写作阶段明确暂停的约束**（步骤 8-10 才执行）：
   - 段落 ≤60 字规则
   - 段落间无空行规则
   ...
   写作时只问自己：读出声顺不顺？相邻 5 段功能类型有变化吗？密度轻→重→轻交替了吗？
7. **正文执行**：如果项目已部署 narrative-writer agent（**必须先检查 `.claude/agents/narrative-writer.md` 是否存在**），spawn `Agent(subagent_type: “narrative-writer”, prompt: “项目目录：{dir}\n任务描述：写正文\n章节：第{N}章\n细纲文件：大纲/细纲_第{N}章.md\n上一章：正文/第{N-1}章_*.md\n准备层输出：{3.1最简记忆包 + 3.2模块/文风召回结果 + 3.3写作意图}\n情绪目标：{从准备层3.3确认}\n涉及角色：{从准备层3.1筛选}\n参考技法：{从准备层3.2召回}\n对标/拆文路径：{本次查找到的 对标/{书名}/ 或 拆文库/{书名}/，没有则写 无}\n对标召回摘要：{准备层3.2(c)输出的相关角色/剧情/设定/章节模块，最多5条；没有则写 无}\n文风路径：{准备层3.2(a) 找到的 文风.md 绝对路径，没有则写 无}\n文风召回指令：{准备层3.2(b) 输出，含匹配章节号和 1-2 句技法指令——例如 '标点节奏照文风文件里的破折号节拍、对话潜台词用问非所答；情绪交替参考第K章爽点铺放比'。没有则写 无}\n原文锚点片段：{文风文件里 4-6 段中按本章情绪选 1-2 段，完整粘贴 300-500字 原文 — 用于 few-shot 模仿手法、非抄字句；没有则写 无}\n写作指南：加载 references/natural-writing-guide.md 作为第一优先级写作总纲。核心要求来自真实网文数据分析——相邻段落的内容密度要有变化（轻→重→轻交替），段落功能类型要多样（动作/感知/闲笔/收束/对话交替，每段不要做一样的事），叙述与角色内心自然融合，用闲笔在紧张场景后制造呼吸感。写完自检：读出声顺不顺？相邻5段的功能类型有变化吗？\n字数硬约束：本章必须达到细纲中设定的字数目标（{从细纲读取}字）。优先使用 Python 字符统计（跨平台）：`python3 -c \”from pathlib import Path; print(len(Path('正文文件路径').read_text(encoding='utf-8')))\”`；macOS/Linux 可用 `wc -m` 备选；禁止使用 `wc -c` 或模型估算。字数未达标禁止结束本章。”)` 执行正文写作，输出写入 `正文/第XXX章_章名.md`。如 narrative-writer agent 未部署，由主线程直接写作。
8. **字数验证**（写作完成后的第一件事）：优先用 `python3 -c “from pathlib import Path; print(len(Path('正文文件路径').read_text(encoding='utf-8')))”` 统计本章实际字数；macOS/Linux 可用 `wc -m` 备选。如果字数 < 细纲目标的 90%，**回到细纲补充更多子事件/情节点**，然后补充正文（参考 natural-writing-guide.md 保持段落内容的多样性），直到字数达标后再进入步骤 9。
9. **检查**：章尾是否有钩子、爽点是否到位
10. **格式收紧 + 禁用词扫描（两遍法·第二遍）**：
    - 10a. 扫破折号（——），替换为句号或逗号
    - 10b. 扫禁用句式（"不是……而是……"及所有变体），改为直接陈述
    - 10c. 对照 `references/banned-words.md`：一级词命中即替换；二级词高频出现时替换，偶发参考 `references/anti-ai-writing.md` 定性裁定
    - 10d. 段落收紧：按 ≤60 字原则切分（少数闲笔/对话可略超，不超 90 字）；段落间去掉空行
    - 10e. 标题格式对齐为 `## 第N章 章名`
    - **第二遍禁止新增情节、设定、关系、时间线。只做格式和措辞调整。**
...
**写作时先加载 `references/natural-writing-guide.md` 作为第一优先级写作总纲。** 当该指南与以下参考文件中的硬性约束（字数切分、强制拆段、高危词列表等）冲突时，以 natural-writing-guide.md 为准。

| 场景 | 加载文件 | 优先级 |
|------|---------|--------|
| **自然流畅写作总纲** | `references/natural-writing-guide.md` | **第一优先级，覆盖所有其他约束** |
...
|------|-----------------|----------------------|
| 爽点（按意图分流） | **`references/plot-emotion-system.md`**（爽点设计体系：本质/六种类型/倒推法） | 翻盘/高潮式爽点→`references/plot-core-methods.md`（假胜→崩解）；打脸/装逼释放→`references/style-combat-face.md`；题材打脸逆袭公式→`references/genre-writing-formulas.md`；爽文循环/多层→`references/outline-methods.md`、`references/outline-conflict.md` |
