...
9. **检查**：章尾是否有钩子、爽点是否到位
10. **模板叠加检测**：对照 `references/banned-words.md` 扫描本章，单用不禁，叠加必杀。同一句中 2+ 模板信号词机械叠加 → 改写；单独使用且有语境功能 → 保留。详细规则见 `references/anti-ai-writing.md`
...
检查两个维度：(1) **情绪交付**——每章是否交付了细纲中规划的目标情绪？(2) **技术质量**——一致性、格式、模板叠加。参考 [references/quality-checklist.md](references/quality-checklist.md) 中的通用检查和长篇专项清单。
...
| 模板叠加检测 | `references/banned-words.md` |
